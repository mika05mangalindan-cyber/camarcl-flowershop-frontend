...
export default function Products() {
  ...
  const applyFilterAndSort = useCallback((filter, sort) => {
    ...
    if (filter.startsWith("category:")) {
      updated = updated.filter(p => p.category === filter.split(":")[1]);
    }
  ...
  const handleSearchChange = useMemo(() =>
    debounce(value => { setSearchQuery(value.toLowerCase()); setCurrentPage(1); }, 300), []
  );
      ...
      autoTable(doc, {
        ...
        didParseCell: data => {
          if (data.section === "body" && data.column.index === 3) {
            ...
            data.cell.styles.textColor = val < 20 ? [220,53,69] : [25,135,84];
          }
        },
      });
  ...
  return (
    <main className="p-4 sm:p-6 lg:p-8">
      ...
      <section className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4 gap-4 p-4 flex-wrap">
