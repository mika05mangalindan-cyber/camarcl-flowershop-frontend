...
const handleDelete = useCallback(async (id) => {
  ...
  return (
    <main className="p-4 sm:p-6 lg:p-8">
      <ProductForm
  key={selectedId || "new"}
  initialData={editMode ? { ...formData, image_url: existingImageUrl } : {}}
  editMode={editMode}
...
//         />
//       ) : (
//         <div className="w-full h-full bg-gray-100 flex items-center justify-center text-gray-400 text-xs italic">
//           No Image
//         </div>
//       )}
//     </div>
...
//         updated = updated.filter(p => p.category === cat);
//       }
//       if (sort === "stock-asc") updated.sort((a, b) => a.stock - b.stock);
//       else if (sort === "stock-desc") updated.sort((a, b) => b.stock - a.stock);
//       else if (sort === "price-asc") updated.sort((a, b) => a.price - b.price);
...
//       applyFilterAndSort(value, sortValue);
//     },
//     [sortValue, applyFilterAndSort]
//   );
...
//     },
//     [filterValue, applyFilterAndSort]
//   );
...
//             const stockValue = Number(data.cell.raw);
//             data.cell.styles.textColor = stockValue < 20 ? [220, 53, 69] : [25, 135, 84];
//           }
//         },
//       });
//       doc.save(`products_report_${date.replace(/[:\/, ]/g, "_")}.pdf`);
...
//         <h1 className="text-xl sm:text-2xl font-semibold text-gray-800">Products</h1>
//         <div className="flex gap-2 items-center flex-wrap">
//           <label htmlFor="filter" className="sr-only">Filter products</label>
//           <select id="filter" value={filterValue} onChange={e => handleFilterChange(e.target.value)} className="border rounded-md px-2 py-1 focus:ring-2 focus:ring-blue-200 text-sm sm:text-base bg-white">
//             <option value="All">All</option>
//             {categories.map((cat, i) => <option key={i} value={`category:${cat}`}>{cat}</option>)}
//           </select>

//           <label htmlFor="sort" className="sr-only">Sort products</label>
//           <select id="sort" value={sortValue} onChange={e => handleSortChange(e.target.value)} className="border rounded-md px-2 py-1 focus:ring-2 focus:ring-blue-200 text-sm sm:text-base bg-white">
//             <option value="none">None</option>
//             <option value="stock-asc">Stock: Low to High</option>
//             <option value="stock-desc">Stock: High to Low</option>
//             <option value="price-asc">Price: Low to High</option>
...
//               <td className="p-3 border-t border-gray-200 flex gap-2 flex-wrap">
//                 <Suspense fallback={null}>
//                   <button onClick={() => handleEdit(product)} className="bg-green-600 hover:bg-green-700 text-white px-3 py-1 rounded-md text-xs flex items-center gap-1 transition-colors duration-150">
//                     <FiEdit2 /> Edit
//                   </button>
//                   <button onClick={() => handleDelete(product.id)} className="bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded-md text-xs flex items-center gap-1 transition-colors duration-150">
//                     <FiTrash2 /> Delete
//                   </button>
//                 </Suspense>
//               </td>
